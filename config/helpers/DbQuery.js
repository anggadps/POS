...
const fieldsWhereMap = (array) => {
    let no = 1, result = '';
    for (const arr in array) result += arr + '=' + `"${array[arr]}" ` + (no == Object.keys(array).length ? "" : "AND" + " "); no++;
    return (Object.keys(array).length > 0 ? " WHERE " : "") + result;
}

const fieldsValueMap = (array) => {
    let no = 1, result = '';
    for (const arr in array) result += arr + '=' + `"${array[arr]}" ` + (no == Object.keys(array).length ? "" : "," + " "); no++;
    return result;
}
